...
const mockedFileHelper = {
  parseCompressedFile: (path: string, _format: string) => {
    if (path.includes('file1')) return [{ id: 1, name: 'Test Data 1' }]
    if (path.includes('file2')) return [{ id: 2, name: 'Test Data 2' }]
...
describe('dataRoutes', () => {
  it('should return parsed data for a valid file ID', async () => {
    ...
    assert.deepStrictEqual(response.body.data, [{ id: 1, name: 'Test Data 1' }])
  })
